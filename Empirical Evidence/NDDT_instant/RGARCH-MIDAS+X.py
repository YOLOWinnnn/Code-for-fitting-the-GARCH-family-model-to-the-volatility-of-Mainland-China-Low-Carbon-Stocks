...
def RGarch_Midas_X(data, indicator):
    #period = 9
    period = 12
    mu0 = np.mean(data['rt'])
    alpha0 = 0.01
    ...
    params0 = [alpha0, beta0, theta0, omega, m0, mu0, w1, xi, psi, tao1, tao2, deta_u]
    bound = [(0, 0.2), (0.7, 1), (0, 1), (-1, 1), (-5, 5), (-1, 1), (1, inf), (-inf, inf), (0, 1),
             (-inf, 0), (0, 1), (0, inf)]
    options = {'maxiter': 3000, 'disp': True}
    myfun = lambda x: -nansum(f_ml(x, data, indic_sample, rt_sample, indicator, rk_sample, period, nobs)[0])
    ...
    index = ['alpha', 'beta', 'theta', 'omega', 'm', 'mu', 'w1', 'xi', 'psi', 'tao1', 'tao2', 'deta_u']
    ...
    aic = (24 - nansum(f_ml(est_params, data, indic_sample, rt_sample, indicator, rk_sample, period, nobs)[0]) * 2) / nobs
    bic = (-2 * nansum(f_ml(est_params, data, indic_sample, rt_sample, indicator, rk_sample, period, nobs)[0])
           + 12 * np.log(nobs)) / nobs
    logger.info('\nAIC : %f\n' % aic)
    logger.info('BIC : %f\n' % bic)
    variance,long_run, short_run = f_ml(est_params, data, indicator_i, rt_full, indicator, rk, period, n)[1:]
...
def f_ml(params, data, indic, rt, indicator, rk, period, nobs):
    alpha0 = params[0]
    beta0 = params[1]
    theta0 = params[2]
    omega0 = params[3]
    ...
    log_l = -0.5 * (log(2 * pi * variance + 1e-3) + zt ** 2) - 0.5 * (
            log(2 * pi * deta_u ** 2 + 1e-3) + ut ** 2 / deta_u ** 2)
    log_l[:loop_start] = 0
...
if __name__ == '__main__':
    ...
    aqi_monthly = data2['sentiment_index']
    print('The program is running, please wait patiently!\n')
    est_params,variance,long_run,short_run =RGarch_Midas_X(data_test, aqi_monthly)
...
d=[yf,Variance_f]
df = pd.DataFrame({'v':yf, 'RGARCH-MIDAS+X':Variance_f},index=date)
